...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Ambil data dari form dengan trim dan default kosong
    ...
    if ($stmt->execute()) {
        ...
        if (!empty($_POST['cast_names']) && !empty($_POST['cast_images'])) {
            ...
            for ($i = 0; $i < count($cast_names); $i++) {
                ...
                if ($name !== '') {
                    $stmt_cast = $conn->prepare("INSERT INTO cast (movie_id, name, image_path) VALUES (?, ?, ?)");
                    if ($stmt_cast) {
                        $stmt_cast->bind_param("iss", $movie_id, $name, $image);
                        $stmt_cast->execute();
                        $stmt_cast->close();
                    }
                }
        ...
        if (!empty($_POST['download_resolutions']) && !empty($_POST['download_paths'])) {
            ...
            for ($i = 0; $i < count($download_resolutions); $i++) {
                ...
                if ($path !== '') {
                    $stmt_dl = $conn->prepare("INSERT INTO downloads (movie_id, resolution, file_path) VALUES (?, ?, ?)");
                    if ($stmt_dl) {
                        $stmt_dl->bind_param("iss", $movie_id, $resolution, $path);
                        $stmt_dl->execute();
                        $stmt_dl->close();
                    }
                }
        ...
        echo json_encode(['success' => false, 'message' => "Gagal menambahkan movie: " . $stmt->error]);
        exit();
    }
} else {
